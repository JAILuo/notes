...
我想到的是Soc `boot` 相关的代码！计算机上电复位后，内部的ROM/RAM 肯定也是写好了程序的，这就是硬件启动的初始状态，需要做什么初始化基础的时钟，从外部介质中读取提前写好的启动程序。

那内核启动也是一样的道理，内核被 `bootloader` 加载到 RAM 中，初始化完成相关的子系统后，之后怎么做？



，但这个时候应该也是有一些文件的，我需要管理！所以在bootloader阶段应该会有一个类似于根文件系统的东西来管理的？就是这个 `initramfs`！
...
但在这个基础上，就能构造出一个像今天一样的Linux的世界（通过很多的 `syscalls`、初始状态 `initramfs`再添加一些多一些内容），各种各样的工具、图形（可能稍微困难点）。。。
