...
### 3.6.7 DSP software stack

前面介绍 DSP +没查资料前，我一直把它当成 SoC 中的一个计算单元来看。

但到了现代移动 SoC，这种理解或许已经不够了。而且，前面似乎一直在讲理论和架构相关的，可能离我这种写代码的稍微有点遥远。所以，本节就简单梳理 DSP 相关的软件平台吧，主要以 Qualcomm Hexagon 为例吧。

首先呢，对于软件工程师而言，我觉得 Hexagon DSP 并不仅仅是一块负责计算的硬件，而是一套完整的软件平台（Software Stack）。

如果把整个软件栈画出来，大概如下：

```C
Application (Android / Linux)
        │
 Hexagon SDK / AI Engine SDK
        │
        FastRPC
        │
      QuRT RTOS
...
说实话，在我查资料之前，我想象中的 DSP 编程可能是：对着数据手册手写汇编，逐条安排 VLIW 槽位，像做 IC 的那些人对着时序图去看去写。

但查完之后发现，**现代 DSP 开发早就不是这样了**。除了极端场景，大部分时候基本也就是在写 C/C++，只是编译器、intrinsics 和 RTOS 的玩法跟通用 CPU 不太一样。

这一节就看看现代 vendor 是怎么做的，重点看高通 Hexagon（毕竟前面 modem 节已经提到了 QuRT），再带一眼 TI 和 Cadence。
