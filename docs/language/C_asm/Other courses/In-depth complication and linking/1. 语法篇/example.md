...
<img src="pic/efc411637eefb4cf8b05213d8a4d4238.jpg" alt="img" style="zoom:33%;" />
...
![img](pic/5c6110c1d93a433d8b9f455e2fa84672.png)
...
- `test` 指令的执行方式与 `cmp` 类似，只不过它会**对传入的两个操作数做隐式的“与”操作，而非减法操作**。
...
- 总结：
    ...
    - 例子：
        ...
        借助 Clang 编译器提供的 `-ast-dump` 选项，我们可以编译并打印出这段 C 代码对应的 AST 结构。完整的编译命令如下：
        ...
        1. 对于上面的 AST 来说，由叶子结点组成的子树需要被最先求值，因此我们首先可以得到括号内加法表达式的计算结果 3。
...
- `if...else`语句：<img src="pic/f0d196f56b772f3b97e6d25e01cd8c46.png" alt="img" style="zoom: 50%;" />
...
- `switch...case` 语句
    ...
    1. 标注为红色的汇编代码会通过 `cmp` 指令，判断寄存器 `eax` 中的值，即变量 v 的值是否大于 60。若判断成立，则直接将程序跳转到标签 .L2 处，并将数字 4 作为返回值；若条件不成立，程序将继续执行。
...
- 优化
    ...
    <img src="pic/1de27b48eb7ceeb09dd81f8500yy97b6.png" alt="img" style="zoom: 33%;" />

    这里我们将 switch…case 语句中分支筛选的“跨度”进行了减小，即将其中的最大分支匹配条件由 60 减小到了 40。跳表是一种用空间换时间的条件匹配策略，让我们通过上图右侧的汇编代码，来了解它的实现过程。

    1. 首先，标注为红色的汇编代码将变量 v 的值减去了选择语句中最小匹配条件的值，这里也就是 10。
...
<img src="pic/9ca51546a4d204c6e5d5da2674c2a928.png" alt="img" style="zoom:50%;" />
...
<img src="pic/0be52e7ff812c7d5384e83c47e9c6233.png" alt="img" style="zoom: 50%;" />
...
![img](pic/435e8e8e1c188698cccbf38e1ec719d4.jpg)
...
- 为什么需要内存数据对齐？
    ...
    对于现代计算机而言，当内存中需要被读写的数据，其所在地址，满足自然对齐的时候，CPU通常能够以**最高的效率进行数据操作**。

    有几个问题：

    - Q1:什么叫做“自然对齐”？

        关于自然对齐，指的是**被操作的数据的所在地址该数据大小的整数倍。**

        比如 `x86-64` 架构中，一个 `int` 类型变量，数据大小为四个字节，其值在内存中连续存放，且，最低有效位字节所在的地址为4的整数倍。
...
- 对于非对齐的数据，为什么某些处理器没法正确访问或者速度下降？其内部硬件原理？
    ...
    - 硬件设计限制
        ...
        > 找到了第 1 篇资料：[单片机(MCU)如何才能不死机之对齐访问(Aligned Access) - 知乎](https://zhuanlan.zhihu.com/p/100616219)
        >
        > 找到了第 2 篇资料：[STM32F0非对齐访问引起的硬件错误及其排除 - 电子工程世界](http://news.eeworld.com.cn/mcu/2019/ic-news021343165.html)
        >
        > 找到了第 3 篇资料：[石锤内存访问不支持非对齐是否STM32H7的硬件bug ...](https://www.armbbs.cn/forum.php?mod=viewthread&tid=94562)
        >
        > 找到了第 4 篇资料：[STM32F0非对齐访问引起的硬件错误及其排除 - CSDN博客](https://blog.csdn.net/fxltsbl007/article/details/52694220)
        >
        > 找到了第 5 篇资料：[【STM32】CortexM0单片机中的非对齐访问造成的HardFault](https://blog.csdn.net/qq_22902757/article/details/106474700)
        >
        > 找到了第 6 篇资料：[STM32F7：访问未对齐的内存引起硬件异常的解决方法 ...](https://www.yiboard.com/thread-646-1-1.html)
        >
        > 找到了第 7 篇资料：[STM32F0非对齐访问引起的硬件错误及其排除 - CSDN博客](https://blog.csdn.net/u010671230/article/details/54383740)
        >
        > 找到了第 8 篇资料：[【经验分享】STM32F0非对齐访问引起的硬件错误及其排除 ...](https://shequ.stmicroelectronics.cn/thread-632063-1-1.html)
        >
        > 找到了第 9 篇资料：[单片机的非对齐访问导致硬件错误_单片机字节不对齐段 ...](https://blog.csdn.net/a369000753/article/details/51584034)
        >
        > 找到了第 10 篇资料：[解析STM32结构体对齐原理和设计原则与实现 - STM32团队 ...](https://shequ.stmicroelectronics.cn/thread-641407-1-1.html)
        >
        > 硬件设计限制通常指的是由于处理器或内存系统的架构特点，对数据访问的地址有特定的要求。如果数据未按照这些要求对齐，就可能导致性能下降或硬件异常。以下是一些实际的例子：
        >
        > 1. **ARM Cortex M0 体系结构**：这种体系结构只支持对齐访问（Aligned Access）。对于4字节（Double Word）型的变量，如果起始地址能被4整除，访问是双字对齐的。如果进行非对齐访问，则可能产生严重错误（HardFault）1。
        > 2. **STM32系列微控制器**：在STM32F0系列中，非对齐访问可能导致硬件错误。例如，如果一个4字节的变量起始地址不能被4整除，编译器可能会在该变量前填充额外的字节以确保对齐，避免访问错误478。
        > 3. **Cortex-M7 内核**：STM32H7系列使用Cortex-M7内核，该内核支持非对齐访问，但在特定条件下，如内存空间被配置为Device或Strongly-ordered模式，非对齐访问可能触发 Usage Fault异常3。
        > 4. **指针访问对齐问题**：在使用指针访问内存时，如果指针的类型和所指向的内存地址未对齐，比如一个`uint32_t`指针指向一个非4字节对齐的地址，访问该地址可能会触发硬件异常135。
        > 5. **内存性能影响**：在一些系统中，非对齐访问可能不会触发硬件异常，但会导致性能下降。因为处理器可能需要进行额外的内存访问和处理步骤来获取未对齐的数据6。
        > 6. **特定内存区域的对齐要求**：某些内存区域，如外部SDRAM，可能对访问地址的对齐有严格要求。未对齐的访问可能导致数据损坏或系统不稳定。
        
    - **内存访问效率**
	
      即使处理器能够访问未对齐的数据，也可能需要进行额外的内存访问和处理步骤。
    
      例如，如果一个32位的 `int` 型数据存储在一个奇数地址开始的地方，处理器可能需要从两个不同的内存地址读取两个16位的数据块，然后内部组合这两个字数据以形成完整的32位数据。
      
      这种访问方式不仅增加了内存访问的次数，而且也增加了处理器的工作量，从而降低了效率
    ...
    - 跨缓存行访问
      ...
      [浅谈CPU内存访问要求对齐的原因 – 仰望苍天思寰宇 (yangwang.hk)](https://yangwang.hk/?p=773)
...
联合与“结构”在语法上的使用方式十分类似，只不过要把对应的语法关键字从 struct 更换为 union 。除此之外，二者还有一个较大的区别，我们可以从“联合”这个名字谈起。顾名思义，“联合”就意味着定义在该结构内的所有数据字段，将会**联合起来共享同一块内存区域**。还是先来看一段代码：

<img src="pic/4609c5e57d1b198884bf5b7d2ee86a18.png" alt="img" style="zoom:50%;" />
...
- 第三行代码将字符 “a” 对应的值 97 存放到结构对象 s 内的匿名联合对象中
    ...
    > ]指令 mov 在进行数据传送时，在目的地参数中使用了 BYTE，也就是“取出”了联合对象所占用的 4 字节空间中的 1 个字节，将其作为存放**字符数值**的目标内存空间。
...
> > GPT:
> >
> > 在 ARM 架构中，与 x86 架构不同，通常不会使用类似 `BYTE`、`QBYTE`、`DBYTE` 这样的指令后缀来指定操作内存中的特定字节或字。ARM 架构是一种精简指令集计算机（RISC），它的指令集设计更加简洁和统一，不像 x86 那样有各种不同长度的数据类型后缀。
> >
> > 在 ARM 架构中，通常使用 `LDR` 和 `STR` 指令来进行内存加载和存储操作。**这些指令可以根据操作数的大小来决定加载或存储多少个字节。**具体来说，可以通过指定不同的寄存器作为操作数，来指定要加载或存储的数据大小。
> >
> > 例如，如果要加载一个字节（8位），可以使用 `LDRB` 指令；如果要加载一个字（32位），可以使用 `LDR` 指令。类似地，存储操作也有相应的指令，如 `STRB` 和 `STR`。
> >
> > 下面是一个简单的示例，展示如何在 ARM 汇编中使用 `LDRB` 和 `STRB` 指令来加载和存储单个字节：
> >
> > ```assembly
> >     LDRB r0, [r1]   ; 从地址 r1 处加载一个字节到寄存器 r0
> >     STRB r0, [r2]   ; 将寄存器 r0 中的字节存储到地址 r2 处
> > ```
> >
> > 如果要加载或存储更多字节，可以使用 `LDR` 和 `STR` 指令，并根据需要选择合适的寄存器和偏移量来指定操作的字节数。
> >
> > 总之，在 ARM 架构中，通常是通过指令本身和操作数寄存器的选择来确定要加载或存储的字节数，而不是像 x86 那样使用特定的数据类型后缀来指定。
>
...
- 结构是一种用于**组织异构数据的复合数据类型**。在结构中，所有定义的数据字段在内存中按顺序排列。为了保证结构中各个字段最高效的数据访问速度，编译器在内存中布局这些字段数据时，会保证它们的起始地址满足自然对齐的标准。因此，结构中字段的不同定义顺序将直接影响结构对象的实际内存占用大小，而这也是我们进行程序优化的一个重要切入点。
- 联合是一种特殊的复合数据类型，在其内部定义的所有数据字段将**占用同一块内存空间**。联合对象的实际大小与其内部所定义最大字段的大小相同。默认情况下，我们无法从外部得知一个联合对象中正在“生效”的字段类型，因此 **Tagged Union** 的使用方式便成为主流。将用来标识生效字段的枚举类型与联合进行“打包”，我们就可以在使用联合对象前进行相应的判断和准备，而这也为应用程序的健壮性打下了基础
...
![image-20240326221322010](pic/image-20240326221322010.png)
...
<img src="pic/image-20240324114648547.png" alt="image-20240324114648547" style="zoom: 50%;" />



#### 基本使用

<img src="pic/3a64f116672998af96b364b616a6e405.png" alt="img" style="zoom:50%;" />
...
<img src="pic/6478bb495988777e77c494192c7d3f02-17112722129277.png" alt="img" style="zoom:50%;" />
...
- 算数
    ...
    <img src="pic/13e25f1000d399993c79a81db8yy34ba.png" alt="img" style="zoom:50%;" />

    定义了一个名为 `arr` 的，具有 2 行 3 列，共 6 个元素的二维数组。

    从右侧对应的汇编代码中，可以看到这个数组内部的数据是以**地址连续**的方式被存放在栈内存中的。**对于这个存储方式，你可以将其理解为编译器对 C 代码中的多维数组进行的扁平化（flatten）处理。**（还是相当于一个一维数组？这不过套了一层？）

    在接下来的 C 代码中，我们通过指针的方式获取并打印了位于数组 `arr` 中两个不同位置上的值。

    - **蓝框**内的表达式首先对 `arr` 进行了加一操作，然后返回了对这个经过“累加”后的地址进行两次解引用的结果值。
        - 从右侧相应的汇编代码中可以看到，对 `arr` 的加一操作导致 `rax` 寄存器中的值被增加 12。而该寄存器中原先存放有数组中第 1 行第 1 列元素对应的地址值，因此在经过计算后，我们得到了一个指向元素 4 的“二级指针”。
        - 也就是说，对变量 `arr` 进行加一操作，导致指向二维数组首元素的指针，向栈中的高地址方向移动了 12 个字节。
        - 之所以会有这样的变化，**是因为 `arr` 在这里直接指向的数据，实际上是二维数组中每一个包含有 3 个整型元素的一维数组。**而每一个一维数组的大小都为固定的 12 字节。因此，当对 `arr` 进行算数运算时，编译器便会以它所指向的一维数组的大小为单位，来进行地址值上的调整。

    - **黄框**内的第二次数组元素访问，由于 `*arr` 作为一级指针（经过了一次解引用），直接指向的是二维数组内某个一维数组中的整型元素，因此，对它进行加法运算，将会以 4 字节作为单位来进行地址上的调整。
...
- 关系运算
    ...
    在大多数情况下，编译器会配合使用 `cmp` 与 `setg` 等指令来判断关系运算符两侧操作数的大小，并根据判断结果，进行相应的置位与复位操作，最终返回 0 或 1 作为结果。

    而对于指针之间的关系运算来说，其实现方式也是如此。但需要注意一点：虽然在机器指令层面，指针的关系运算实际上是对指针内部所存放的地址值进行的大小判断，但从 C 语法的角度来看，具有实际意义的指针关系运算仅有为数不多的几种情况（[Comparison operators - cppreference.com](https://en.cppreference.com/w/c/language/operator_comparison)）。除此之外，其他使用方式均会产生未定义行为（UB）。
...
在之前介绍的例子中，指针仅引用了位于栈内存中的数据。但实际上，指针还有另一个更重要的作用，那就是给予了我们**灵活操控堆内存中数据的能力**。
...
- 下面这段代码正常编译吗？
    ...
    编译通过，但是可读性差，实际代码中不要使用。

    都表示同样的东西，直接进行判断为1，为什么？

    

    ![image-20240324231803442](pic/image-20240324231803442.png)

    > **下标运算符[] 的定义就是用于两个操作数，一个为指针，一个为整数，但并没有定义顺序关系，所以`arr[3]`当然等于 `[3]arr`  这是c语言规范定义的**。
    >
    > `arr` 是数组名，是数组对象的标识符，在这边用于表达式时转为指向首元素的指针，因为 `arr` 是二维数组，所以此时 `arr` 转换为指针后，指向子数组{1,2,3}。`arr+3` 范围超过了 `arr` 数组的范围，地址应该是 `arr的地址值+sizeof(int)×3×3`
...
<img src="pic/image-20240324232340001.png" alt="image-20240324232340001" style="zoom: 67%;" />
...
- **技巧二：为宏函数的参数添加括号**
    ...
    在这里，我们改变了宏函数 FOO 的使用方式，直接将表达式 1 + 2 作为参数传递给了它。

    由于编译器在处理宏函数时，仅会进行实参在各自位置上的文本替换，传入函数的表达式并不会在函数展开前进行求值。

    因此，经过编译器的预处理后，上述代码中第四行对 `printf` 语句的调用过程会被变更为如下形式：
...
- **技巧四：定义完备的多语句宏函数**
    ...
    当我们习惯使用 C 代码的执行思维来看待宏函数时，上述代码的执行情况应该是这样的：程序接收用户输入的字符，并将其转换为数字值。若该值大于 0，则宏函数内的两条 `printf` 语句被执行，并输出字符串 “Hello, world!”。否则，程序直接退出。

    但现实的情况却是，无论用户输入何值，字符串 “world!” 都会被打印。而问题就出现在宏函数 SAY 被展开和替换后，原本“封装”在一起的两条 `printf` 语句被拆分开来。其中的第一条语句成为了 if 条件控制语句的执行内容；而第二条语句由于没有大括号的包裹，则直接被“释放”到了 main 函数中，成为了该函数返回前最后一条会被执行的语句。
...
- C 预处理运算符 “#” 与 “##” 的用法
    ...
    1. **# 运算符**：
       ...
       - 例如，以下是一个简单的宏定义：
         ...
         ```c
         printf("%s", STR(test)); // 输出字符串 "test"
         ```
         
         **==运算符的作用是将宏参数转换为字符串常量，这在一些宏定义中非常有用，可以将参数转换为字符串进行处理。==**
    ...
    2. **## 运算符**：
       ...
       - 例如，以下是一个简单的宏定义：
         ...
         ```c
         int num = CONCAT(10, 20); // 相当于 int num = 1020;
         ```
         
         **==运算符的作用是将两个标识符连接在一起，可以用于动态生成新的标识符，这在一些复杂的宏定义中非常有用。==**
